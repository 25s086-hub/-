...
tone = st.selectbox("답변의 말투를 골라", ["친절", "냉철", "유머"])
features = st.multiselect("추가 기능을 선택해라", ["이미지 생성", "웹 검색", "코드 분석", "번역"])
creativity = st.slider("AI의 창의성 수준을 설정해라", 0, 100, 50)
ai_speed = st.select_slider("응답 처리 속도를 선택해라",options=["매우 느림", "느림", "보통", "빠름", "실시간"],value="보통")
agree = st.checkbox("개인정보 수집 및 AI 학습 이용에 동의해.")
st.markdown("---")
